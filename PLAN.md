...
| 2 | Tue Sep 16 | **You:** sign-up form, create the GitHub repo, push. **Me (done early, Sep 15 night):** local harden of the scaffold on cmos5l through routing and GDS (baseline in README); SRAM-macro trial (fails at PDN, cause and fix identified). **Remaining:** push so the `gds`, `test`, `docs` workflows run the full flow incl. Magic/precheck | workflows green; macro fix planned |
...
| D6 | **2026-09-16: spec v1 and N1–N10 approved by you.** Implementation plan for core v0: `docs/superpowers/plans/2026-09-16-core-v0.md` (10 tasks: ISA generator, assembler, simulator, timebase/GPIO, core, host SPI port, differential test, UART equivalence, formal proofs, harden). GitHub repo: https://github.com/Abagel-coder/protocol-emulator. Earlier: research done (2026-09-15 evening): `docs/research/prior-art-academic.md`, `prior-art-industry.md`, `verification-novelty.md`, distilled into `docs/research/novelty-synthesis.md` with decisions N1–N10. The design spec is now v1 and incorporates them; **N1–N10 and the v1 spec await your approval.** Headline: the timing contract is a machine-checked theorem; lanes carry line coding, bit-stuffing and CRC; contention monitor and edge-timestamp capture; every wait is bounded. **Core v0 done on 2026-09-18: 12,757 cells, 21.39% utilisation, +2.58 ns setup slack; next plan: lanes (pin engines).** |
...
| SRAM macro on CMOS5L: trial fails at power-grid generation (Metal4 stripes vs Metal4 macro pins; see research brief) | Instruction memory strategy depends on it | Week 2: port PRISM's stripe-alignment approach or add TopMetal1 straps; ask on the Tiny Tapeout Discord; fallback is a 64–128 word flip-flop instruction memory |
| Docker/LibreLane on Apple Silicon | Local hardening is fragile: under heavy I/O the Docker VM's file-sharing service ("fs": virtiofs event injection) blocks for 60 s and Docker Desktop stops the engine mid-run (2026-09-15: at global placement and CTS with 8 GB, and at the final Magic LEF step with 12 GB, after routing and GDS export had succeeded) | VM memory raised to 12 GB; next options: switch Docker Desktop's file-sharing implementation (Settings → General) or put `~/ttsetup/pdk` and the run directory on a Docker volume; treat GitHub Actions as the flow of record; local runs for iteration up to routing |
