...
| `0x02` | `WRITE_CTRL` | one byte: bit0 = RUN (ORed with the synchronised RUN pin), bit1 = RESET_CORE (pulse: clears PC, flags, halted) |
| `0x03` | `READ_STATUS` | host clocks 4 dummy bytes, receives `status pc_hi pc_lo flags`; `status` bit0 = halted, bit1 = running, bit2 = fifo_rx_valid (h2c_valid), bit3 = fifo_tx_empty (!c2h_valid), bit4 = fifo_rx_full (h2c_full); `flags` bit0 Z, bit1 C, bit2 TO |
| `0x04` | `PUSH_FIFO` | `data_hi data_lo` → host→core FIFO (read by `POP`); dropped (no-op) if the FIFO is already full -- see `READ_STATUS` bit4 |
| `0x05` | `POP_FIFO` | host clocks 2 dummy bytes, receives `data_hi data_lo` from the core→host FIFO (written by `PUSH`) |
| `0x06` | `READ_GPIO` | host clocks 5 dummy bytes, receives a *synchronised* snapshot `ui_in uio_in uo_out uio_out uio_oe` (`ui_in`/`uio_in` are the GPIO block's two-flop-synchronised `ui_sync`/`uio_sync`, not the raw top-level pins) |
...
RUN pin synchronisation: `ui_in[7]` (RUN) is an asynchronous top-level input. It passes through the GPIO block's two-flop input synchroniser before reaching the core's own registered RUN sample (`run_q`), so it takes three clock cycles total from a RUN pin transition to the first instruction executing (2 sync flops + 1 core RUN register). `WRITE_CTRL`'s run bit is already a register in the clock domain and does not go through this synchroniser, so it can take effect one cycle sooner than the pin.
